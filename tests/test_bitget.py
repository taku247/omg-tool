...
async def test_bitget_rest_api():
    """Bitget REST API テスト"""
    
    print("=" * 60)
    print("🟠 Bitget REST API テスト")
    print("=" * 60)
    
    exchange = BitgetExchange(testnet=False)
    ...
    try:
        ...
        for unified, expected in test_conversions:
            bitget_symbol = exchange._convert_symbol_to_bitget(unified)
            back_to_unified = exchange._convert_symbol_from_bitget(bitget_symbol)
            
            if bitget_symbol == expected and back_to_unified == unified:
                print(f"✅ {unified} <-> {bitget_symbol}")
                conversion_success += 1
            else:
                print(f"❌ {unified} -> {bitget_symbol} (expected {expected})")
        ...
        print(f"❌ Bitget REST APIテストエラー: {e}")
        logger.error(f"Bitget REST API test failed: {e}", exc_info=True)
...
async def test_bitget_websocket():
    """Bitget WebSocket テスト（短時間）"""
    
    print("\n" + "=" * 60)
    print("🔌 Bitget WebSocket テスト")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n❌ WebSocketテストエラー: {e}")
        logger.error(f"Bitget WebSocket test failed: {e}", exc_info=True)
        return False
...
async def main():
    ...
    print("🧪 Bitget取引所実装テスト")
    print("=" * 60)
    
    # REST APIテストを実行
    rest_success = await test_bitget_rest_api()
    ...
    if rest_success:
        ...
        ws_success = await test_bitget_websocket()
