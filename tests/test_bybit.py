...
async def test_bybit_rest_api():
    """Bybit REST API テスト"""
    
    print("=" * 60)
    print("🟡 Bybit REST API テスト")
    print("=" * 60)
    ...
    try:
        ...
        for unified, expected in test_conversions:
            bybit_symbol = exchange._convert_symbol_to_bybit(unified)
            back_to_unified = exchange._convert_symbol_from_bybit(bybit_symbol)
            
            if bybit_symbol == expected and back_to_unified == unified:
                print(f"✅ {unified} <-> {bybit_symbol}")
                conversion_success += 1
            else:
                print(f"❌ {unified} -> {bybit_symbol} (expected {expected})")
        ...
        print(f"❌ Bybit REST APIテストエラー: {e}")
        logger.error(f"Bybit REST API test failed: {e}", exc_info=True)
...
async def test_bybit_websocket():
    """Bybit WebSocket テスト（短時間）"""
    
    print("\n" + "=" * 60)
    print("🔌 Bybit WebSocket テスト")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n❌ WebSocketテストエラー: {e}")
        logger.error(f"Bybit WebSocket test failed: {e}", exc_info=True)
        return False
...
async def main():
    ...
    # REST APIテストを実行
    rest_success = await test_bybit_rest_api()
    ...
    if rest_success:
        ...
        ws_success = await test_bybit_websocket()
