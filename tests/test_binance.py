...
async def test_binance_rest_api():
    """Binance REST API テスト"""
    
    print("=" * 60)
    print("🟡 Binance REST API テスト")
    print("=" * 60)
    ...
    try:
        ...
        for unified, expected in test_conversions:
            binance_symbol = exchange._convert_symbol_to_binance(unified)
            back_to_unified = exchange._convert_symbol_from_binance(binance_symbol)
            
            if binance_symbol == expected and back_to_unified == unified:
                print(f"✅ {unified} <-> {binance_symbol}")
                conversion_success += 1
            else:
                print(f"❌ {unified} -> {binance_symbol} (expected {expected})")
        ...
        print(f"❌ Binance REST APIテストエラー: {e}")
        logger.error(f"Binance REST API test failed: {e}", exc_info=True)
...
async def test_binance_websocket():
    """Binance WebSocket テスト（短時間）"""
    
    print("\n" + "=" * 60)
    print("🔌 Binance WebSocket テスト")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n❌ WebSocketテストエラー: {e}")
        logger.error(f"Binance WebSocket test failed: {e}", exc_info=True)
        return False
...
async def main():
    ...
    print("🧪 Binance取引所実装テスト")
    print("=" * 60)
    
    # REST APIテストを実行
    rest_success = await test_binance_rest_api()
    ...
    if rest_success:
        ...
        ws_success = await test_binance_websocket()
