...
async def test_kucoin_rest_api():
    """KuCoin REST API テスト"""
    
    print("=" * 60)
    print("🔵 KuCoin REST API テスト")
    print("=" * 60)
    
    exchange = KuCoinExchange(testnet=False)
    ...
    try:
        ...
        for unified, expected in test_conversions:
            kucoin_symbol = exchange._convert_symbol_to_kucoin(unified)
            back_to_unified = exchange._convert_symbol_from_kucoin(kucoin_symbol)
            
            if kucoin_symbol == expected and back_to_unified == unified:
                print(f"✅ {unified} <-> {kucoin_symbol}")
                conversion_success += 1
            else:
                print(f"❌ {unified} -> {kucoin_symbol} (expected {expected})")
        ...
        print(f"WebSocketエンドポイントURL: {exchange.ws_endpoint_url}")
        print(f"REST URL: {exchange.rest_url}")
        success_count += 1
            
    except Exception as e:
        print(f"❌ KuCoin REST APIテストエラー: {e}")
        logger.error(f"KuCoin REST API test failed: {e}", exc_info=True)
...
async def test_kucoin_websocket():
    """KuCoin WebSocket テスト（短時間）"""
    
    print("\n" + "=" * 60)
    print("🔌 KuCoin WebSocket テスト")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n❌ WebSocketテストエラー: {e}")
        logger.error(f"KuCoin WebSocket test failed: {e}", exc_info=True)
        return False
...
async def main():
    ...
    print("🧪 KuCoin取引所実装テスト")
    print("=" * 60)
    
    # REST APIテストを実行
    rest_success = await test_kucoin_rest_api()
    ...
    if rest_success:
        ...
        ws_success = await test_kucoin_websocket()
