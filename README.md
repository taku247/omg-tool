...
-   6 取引所からの価格データを 1 秒間隔で CSV に記録
-   日付別・取引所別にファイル自動ローテート
-   bid/ask 価格、サイズ、出来高などの詳細情報を保存

**2. backtest_engine.py** - バックテスト実行エンジン
...
# デフォルト実行（Binance除外で安定動作）
python price_logger.py --symbols BTC ETH SOL

# Binanceを含める場合（高負荷になる可能性あり）
python price_logger.py --symbols BTC ETH SOL --exchanges Hyperliquid Bybit Binance Gateio Bitget KuCoin
...
| チェック項目                                   | 現状                                                                                          | バックテストへの影響                                                                       |
| ---------------------------------------------- | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| **必須列** `timestamp,exchange,symbol,bid,ask` | ✅ そろっている                                                                               | 乖離計算に十分                                                                             |
| ISO-8601 タイムスタンプ（+00:00 付き）         | ✅ ミリ秒精度あり                                                                             | 取引所間で時系列マージしやすい                                                             |
| **複数取引所行が同一時刻に存在**               | ❔（サンプルは Binance のみ）                                                                 | *必ず他取引所も同時に記録*しないと乖離判定できない                                         |
| 1 秒間隔で連続行                               | △ 「価格が変わった時だけ書く」差分ログなので**変化がない秒は空行** → 後でリサンプリングが必要 |                                                                                            |
| `bid_size / ask_size`                          | 空欄でも可                                                                                    | スリッページ推定を後付けするなら埋めても良い                                               |
| 大きなタイムギャップ（例 22:14 → 22:32）       | 差分ログの仕様通り                                                                            | ギャップ部分は「価格が動かなかった」扱い。後で `df.resample('1S').ffill()` で補完すれば OK |
